...
Geom | Data Requirements | Bioinformatics Applications | Key Parameters
-----|------------------|----------------------------|---------------
`geom_point()` | Continuous x, y | PCA plots, Volcano plots | `alpha=0.5` (for overlapping points), `size`
`geom_tile()` | Discrete x, y | Heatmaps, Correlation matrices | `aes(fill = value)`
`geom_boxplot()` | Discrete x, Continuous y | Gene expression comparison across groups | `outlier.shape = NA` (hide outliers for overlay)
`geom_violin()` | Discrete x, Continuous y | Marker gene distribution in scRNA-seq clusters | `trim = FALSE`, `scale = "width"`
`geom_bar()` | Discrete x | Sample classification, GO/KEGG enrichment | `stat = "identity"` (when y values are pre-calculated)
`geom_density2d()` | Continuous x, y | Contour density for large scRNA-seq trajectories | `color = "blue"`
`geom_jitter()` | Discrete x, Continuous y | Overlay raw data points on boxplots | `width = 0.2`, `alpha = 0.3`
