...
def main():
    ...
    # Verification 6: FoldChange consistency with log2FoldChange
    # For non-infinite values, check that 2^log2FC ≈ FC
    finite_mask = ~(df_final['FoldChange'].isin([float('inf'), float('-inf')]) | 
                   df_final['log2FoldChange'].isin([float('inf'), float('-inf')]))
    if finite_mask.sum() > 0:
        calculated_fc = 2 ** df_final.loc[finite_mask, 'log2FoldChange']
        actual_fc = df_final.loc[finite_mask, 'FoldChange']
        # Allow small numerical differences
        if (abs(calculated_fc - actual_fc) < 1e-10).all():
            print("✅ FoldChange consistency verification: log2FoldChange matches FoldChange")
        else:
            print("❌ FoldChange consistency verification FAILED")
            sys.exit(1)
